...
describe( 'snapwp main execution', () => {
	beforeEach( () => {
		...
		( require( 'path' ).resolve as jest.Mock ).mockImplementation(
	...
	it( 'should run the full project creation flow successfully', async () => {
		...
		expect( createProjectDirectory ).toHaveBeenCalledWith(
			resolvedTestProjectDir
		);
		expect( setupEnvFile ).toHaveBeenCalledWith(
			resolvedTestProjectDir,
			false
		); // false for useDefaultEnv
		expect( copyStarterTemplate ).toHaveBeenCalledWith(
			resolvedTestProjectDir
		);
		expect( setupNpmrc ).toHaveBeenCalledWith(
			resolvedTestProjectDir,
			undefined
		); // undefined for proxy option
		expect( updatePackageVersions ).toHaveBeenCalledWith(
			resolvedTestProjectDir
		);
		expect( runNpmInstall ).toHaveBeenCalledWith( resolvedTestProjectDir );
		expect( printSuccessMessage ).toHaveBeenCalledWith(
			resolvedTestProjectDir,
			false,
			false
		); // useDefaultEnv, needsManualInstall
	...
	it( 'should skip npm install if --skip-install flag is provided', async () => {
		...
		expect( mockConsoleLog ).toHaveBeenCalledWith(
			'Skipping NPM dependencies installation...'
		);
		expect( printSuccessMessage ).toHaveBeenCalledWith(
			expect.any( String ),
			false,
	...
	it( 'should set needsManualInstall to true if runNpmInstall throws an error', async () => {
		( runNpmInstall as jest.Mock ).mockRejectedValue(
	...
	it( 'should handle errors during project creation and exit', async () => {
		...
		( createProjectDirectory as jest.Mock ).mockRejectedValue(
