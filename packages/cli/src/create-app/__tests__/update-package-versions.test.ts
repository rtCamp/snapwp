...
const mockCliPackageJsonPath = path.resolve(__dirname, '../../../../package.json');
...
describe('updatePackageVersions', () => {
  ...
  it('should update @snapwp/* package versions in dependencies', () => {
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((filePath) => {
      if (filePath === projectPackageJsonPath) return JSON.stringify(projectPackageJson);
      if (filePath === mockCliPackageJsonPath) return JSON.stringify(cliPackageJsonContent);
      return '';
    });
  ...
  it('should add carets to versions that do not have them', () => {
    ...
    expect(writtenContent.dependencies['@snapwp/core']).toBe(`^${mockCliVersion}`);
  });
  ...
  it('should throw an error if reading project package.json fails', () => {
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((filePath) => {
      if (filePath === projectPackageJsonPath) throw readError;
      if (filePath === mockCliPackageJsonPath) return JSON.stringify(cliPackageJsonContent);
      return '';
    });
  ...
  it('should throw an error if writing project package.json fails', () => {
    const projectPackageJson = { name: 'test', version: '1.0.0', dependencies: { '@snapwp/core': '0.1.0' } };
    (fs.readFileSync as jest.Mock).mockImplementation((filePath) => {
      if (filePath === projectPackageJsonPath) return JSON.stringify(projectPackageJson);
      if (filePath === mockCliPackageJsonPath) return JSON.stringify(cliPackageJsonContent);
      return '';
   ...
   it('should throw an error if reading CLI package.json fails', () => {
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((filePath) => {
      if (filePath === mockCliPackageJsonPath) throw readCliError;
