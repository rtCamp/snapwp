...
| `customPaths`    | `Record<string, MetadataRoute.Sitemap>` | `undefined`       | Record of string key and MetadataRoute.Sitemap as it's value where each key is the name of a sub-sitemap (e.g., `wp-sitemap-posts-page-1`) and the value is a array of `MetadataRoute.Sitemap` object. **Important:** If the WordPress sub-sitemap URL is something like `http://snapwp-helper.local/wp-sitemap-posts-page-1.xml`, the key should be `wp-sitemap-posts-page-1`.                                                                    |
