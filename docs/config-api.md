...
| `customPaths`    | `Record<string, SitemapData[]>` | `undefined`       | Record of string key and array of SitemapData as it's value where each key is the name of a sub-sitemap (e.g., `wp-sitemap-posts-page-1`) and the value is a array of SitemapData object. **Important:** If the WordPress sub-sitemap URL is something like `http://snapwp-helper.local/wp-sitemap-posts-page-1.xml`, the key should be `wp-sitemap-posts-page-1`.                                                                                 |

## SitemapData is a subset of `MetadataRoute.Sitemap` with following properties omitted:

| **Property** | **Description**                          |
| ------------ | ---------------------------------------- |
| videos       | Currenty not supported by SnapWP Sitemap |
| images       | Currenty not supported by SnapWP Sitemap |
